...
//     // Scan through table looking for exited children.
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      // if(cas((int*)(&p->state), ZOMBIE, UNUSED)){//c_check -> TODO: problem with this transition at usertests!
        ...
        // kfree(p->kstack);
        // p->kstack = 0;
        // freevm(p->pgdir); TODO: why comment this helps?!?
        p->pid = 0;
        p->parent = 0;
        p->name[0] = 0;
    ...
    if(!havekids || curproc->killed){
      ...
      if(!cas((int*)(&curproc->state), MINUS_SLEEPING, RUNNING)){
        panic("error in wait: should be at minus_sleeping state!");
      }
      popcli();
      // release(&ptable.lock);
      return -1;
  ...
  for(;;){
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        if(p->state == MINUS_ZOMBIE){
            ...
            freevm(p->pgdir);
            p->killed = 0;
            p->chan = 0;  // TODO: why move it here?
...
static char *states[] = {
  ...
  // int i; TODO why uncomment?
  struct proc *p;
  char *state;
  // uint pc[10]; TODO: why uncooment?
...
sigkill_handler(int sig){
  ...
  cas((int*)(&p->state), SLEEPING, MINUS_RUNNABLE);
...
user_handler(struct proc* p, uint sig)
{
  ...
  //   // uint sigret_size;
  //   memmove(&p->tf_backup, p->tf, sizeof(struct trapframe));//backing up trap frame
  //   p->tf->esp -= sigret_caller_end - sigret_caller_start; // make room for sigret code
  //   memmove((void*)p->tf->esp, sigret_caller_start,  sigret_caller_end - sigret_caller_start);
...
//         {
//           p->frozen = 0;
//           // p->psignals ^= 1 << SIGCONT; we ignore sig_dfl on kill!
//         }
//         else 
//           continue;
//       }
